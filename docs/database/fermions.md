...
**5 files / 48 Qed.** Score distribution: s5=0 / s4=0 / s3=0 / s2=1 / s1=4 / s0=0
...
- **Role.** Leaf of the fermions (SM-from-graph) branch. Self-contained (QArith). SM-from-distinction is OVER-BRANDED.
...
| name | kind | role |
|---|---|---|
| `dirac_eigenvalue_sq/fermion_propagator_sq/massless_zero_mode/massive_gap/doubler_heavy` | Definition/Theorem | ★ спектр Дирака, нулевая мода, дублёры |
| `eigenvalue_k1_N4/propagator_value/eigenvalue_grows_with_k/_with_mass/chirality_from_L2` | Theorem | рост собственных значений, киральность |
| `dirac_antisymmetric/n4_spectrum_check/dirac_on_graph_synthesis` | Theorem | антисимметрия, проверка спектра |
...
- **Role.** Diagnostic leaf of the fermions/Higgs branch. Self-contained.
...
| name | kind | role |
|---|---|---|
| `top_drives_negative/gauge_drives_positive/total_correction_positive/propagator_finite` | Theorem | ★ top−, gauge+, итог положителен |
| `mass_hierarchy/dirac_spectrum_from_graph/yukawa_from_L2/higgs_diagnostic_synthesis` | Theorem | иерархия масс, Юкава из L2 |
...
- **Role.** Leaf of the fermions/Higgs-correction branch. Self-contained.
- **Counts.** Qed 11 / Admitted 0 / axioms 0
- **Imports.** Stdlib: QArith
...
| name | kind | role |
|---|---|---|
| `N_c/top_loop_sum_4/delta_mH_sq/mH_sq_tree` | Definition | цвета, петлевая сумма, древесная масса |
| `top_loop_positive/top_loop_value/top_loop_negative/top_correction_value/tree_plus_top` | Theorem | ★ значение и знак top-поправки |
| `top_loop_at_m1/top_correction_at_m1/top_loop_positive_m1/grows_with_N/need_gauge/top_loop_synthesis` | Theorem | рост с N_c, нужны gauge-петли |
...
- **Topic.** The observed top and bottom Yukawa couplings, top dominance, fermion mass from Yukawa, top Yukawa ~1, bottom negligible, the mass ratio, and 'Yukawa is L2'.
- **Role.** Leaf of the fermions branch (mass generation). Self-contained.
- **Counts.** Qed 9 / Admitted 0 / axioms 0
- **Imports.** Stdlib: QArith
...
| `bottom_mass_small/mass_ratio/top_dominance_positive/yukawa_is_L2/yukawa_coupling_synthesis` | Theorem | малость bottom, Юкава=L2 |
