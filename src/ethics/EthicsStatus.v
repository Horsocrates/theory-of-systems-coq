...
Definition pravilno (thr fit : nat) : Prop := thr <= fit.

(** Статус «правильно/неправильно» РЕШАЕМ (не интуиция-оракул). *)
Theorem pravilno_dec : forall thr fit, {pravilno thr fit} + {~ pravilno thr fit}.
Proof.
  intros thr fit. unfold pravilno. destruct (thr <=? fit) eqn:E.
...
Theorem pravilno_or_not : forall thr fit, pravilno thr fit \/ ~ pravilno thr fit.
Proof. intros thr fit. destruct (pravilno_dec thr fit); [left|right]; assumption. Qed.

(** Булева реализация гейта и её адекватность. *)
Definition passes (thr fit : nat) : bool := thr <=? fit.

Theorem passes_reflect : forall thr fit, passes thr fit = true <-> pravilno thr fit.
Proof. intros thr fit. unfold passes, pravilno. apply Nat.leb_le. Qed.
...
Theorem in_pool_iff : forall thr l v,
  In v (pool thr l) <-> In v l /\ pravilno thr v.
Proof.
...
Theorem pool_pravilno : forall thr l v, In v (pool thr l) -> pravilno thr v.
...
Theorem pool_nil_iff : forall thr l,
  pool thr l = [] <-> (forall v, In v l -> ~ pravilno thr v).
Proof.
  intros thr l. split.
  - intros H v Hin Hp.
...
Example mult_pravilno_example : tie_at [5; 5; 3] 5.
Proof.
  exists 0, 1. split; [discriminate | split; reflexivity].
Qed.

(** Ничья ⇒ свобода выбора (P4): оба варианта правильны при пороге ≤ best. *)
Theorem tie_both_pravilno : forall l best thr,
  pravilno thr best -> tie_at l best ->
  pravilno thr best /\ pravilno thr best.
...
Theorem threshold_then_height : forall thr l v,
  In v (pool thr l) -> pravilno thr v /\ v <= maxfit (pool thr l).
Proof.
  intros thr l v H. split.
  - exact (pool_pravilno thr l v H).
