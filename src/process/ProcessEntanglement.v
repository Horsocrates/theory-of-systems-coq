...
(** Concrete entangled Rule: correlates same-index pairs *)
(** R(a1,b1,a2,b2) = 1 if a1=a2 and b1=b2, or a1!=a2 and b1!=b2 *)
(** This is the Bell pair interaction *)
Definition bell_rule : CompositeRule :=
  fun a1 b1 a2 b2 =>
    if (Nat.eqb a1 a2 && Nat.eqb b1 b2)%bool then 1
    else if (negb (Nat.eqb a1 a2) && negb (Nat.eqb b1 b2))%bool then 1
    else 0.

(** Compute bell_rule values *)
Lemma bell_rule_00_00 : bell_rule 0 0 0 0 == 1.
Proof. unfold bell_rule. simpl. reflexivity. Qed.

Lemma bell_rule_00_11 : bell_rule 0 0 1 1 == 1.
Proof. unfold bell_rule. simpl. reflexivity. Qed.

Lemma bell_rule_01_01 : bell_rule 0 1 0 1 == 0.
Proof. unfold bell_rule. simpl. reflexivity. Qed.

Lemma bell_rule_01_10 : bell_rule 0 1 1 0 == 1.
Proof. unfold bell_rule. simpl. reflexivity. Qed.

Lemma bell_rule_11_11 : bell_rule 1 1 1 1 == 1.
Proof. unfold bell_rule. simpl. reflexivity. Qed.

Lemma bell_rule_10_01 : bell_rule 1 0 0 1 == 1.
Proof. unfold bell_rule. simpl. reflexivity. Qed.
...
Theorem bell_rule_entangled : is_entangled bell_rule 2 2.
Proof.
  unfold is_entangled, is_separable. intros [fA [fB Hsep]].
  (* Get key equations *)
  assert (H0000 := Hsep 0 0 0 0 ltac:(lia) ltac:(lia) ltac:(lia) ltac:(lia)).
  assert (H0101 := Hsep 0 1 0 1 ltac:(lia) ltac:(lia) ltac:(lia) ltac:(lia)).
  assert (H1111 := Hsep 1 1 1 1 ltac:(lia) ltac:(lia) ltac:(lia) ltac:(lia)).
  (* Compute bell_rule values *)
  rewrite bell_rule_00_00 in H0000.
  rewrite bell_rule_01_01 in H0101.
  rewrite bell_rule_11_11 in H1111.
  (* From H0000: fA 0 0 * fB 0 0 == 1, so both nonzero *)
  assert (HfA00 : ~ fA 0 0 == 0).
  { intros Heq. rewrite Heq in H0000.
    rewrite Qmult_0_l in H0000. lra. }
  (* From H0101: fA 0 0 * fB 1 1 == 0 *)
  (* Since fA 0 0 != 0: fB 1 1 == 0 *)
  assert (HfB11 : fB 1 1 == 0).
  { destruct (Qeq_dec (fB 1 1) 0) as [Heq | Hneq].
    - exact Heq.
    - exfalso. apply HfA00.
      destruct (Qeq_dec (fA 0 0) 0) as [Heq0 | Hneq0].
      + exact Heq0.
      + exfalso.
        assert (Hprod : fA 0 0 * fB 1 1 == 0) by lra.
        destruct (Qmult_integral _ _ Hprod) as [Ha | Hb].
        * contradiction.
        * contradiction. }
  (* From H1111: fA 1 1 * fB 1 1 == 1, but fB 1 1 == 0 *)
...
(** Bell state values *)
Lemma bell_state_00 : qi_eq (bell_state 0 0) (mkQi (5 # 7) 0).
Proof. unfold bell_state, qi_eq. simpl. split; reflexivity. Qed.

Lemma bell_state_01 : qi_eq (bell_state 0 1) qi_zero.
Proof. unfold bell_state, qi_eq, qi_zero. simpl. split; reflexivity. Qed.

Lemma bell_state_10 : qi_eq (bell_state 1 0) qi_zero.
Proof. unfold bell_state, qi_eq, qi_zero. simpl. split; reflexivity. Qed.

Lemma bell_state_11 : qi_eq (bell_state 1 1) (mkQi (5 # 7) 0).
Proof. unfold bell_state, qi_eq. simpl. split; reflexivity. Qed.
...
Theorem bell_state_entangled : state_entangled bell_state 2 2.
Proof.
  unfold state_entangled, state_separable.
  intros [alpha [beta Hsep]].
  (* Get equations *)
  assert (H00 := Hsep 0 0 ltac:(lia) ltac:(lia)).
  assert (H01 := Hsep 0 1 ltac:(lia) ltac:(lia)).
  assert (H11 := Hsep 1 1 ltac:(lia) ltac:(lia)).
  ...
  (* From H00r: qi_re(alpha 0) * qi_re(beta 0) - qi_im(alpha 0) * qi_im(beta 0) == 5#7 *)
  (* From H01r: qi_re(alpha 0) * qi_re(beta 1) - qi_im(alpha 0) * qi_im(beta 1) == 0 *)
  (* From H01i: qi_re(alpha 0) * qi_im(beta 1) + qi_im(alpha 0) * qi_re(beta 1) == 0 *)
  (* From H11r: qi_re(alpha 1) * qi_re(beta 1) - qi_im(alpha 1) * qi_im(beta 1) == 5#7 *)
  (* Need: qi_norm2 (alpha 0) * qi_norm2 (beta 1) == 0 from H01 *)
  (* But qi_norm2 (alpha 0) > 0 from H00 and qi_norm2 (beta 1) > 0 from H11 *)
  (* Use: |alpha(0)*beta(1)|^2 = |alpha(0)|^2 * |beta(1)|^2 *)
  (* |alpha(0)*beta(1)| = |psi(0,1)| = 0 *)
  (* So |alpha(0)|^2 * |beta(1)|^2 = 0 *)
  assert (Hnorm01 : qi_re (alpha 0) * qi_re (beta 1) - qi_im (alpha 0) * qi_im (beta 1) == 0).
  { exact H01r. }
  assert (Hnorm01i : qi_re (alpha 0) * qi_im (beta 1) + qi_im (alpha 0) * qi_re (beta 1) == 0).
  { exact H01i. }
  (* |alpha(0)|^2 * |beta(1)|^2 = (re*re+im*im) * (re'*re'+im'*im') *)
  (* = (re*re'-im*im')^2 + (re*im'+im*re')^2 = 0^2 + 0^2 = 0 *)
  assert (Hprod : qi_norm2 (alpha 0) * qi_norm2 (beta 1) == 0).
  { unfold qi_norm2.
    assert (Hkey : (qi_re (alpha 0) * qi_re (alpha 0) + qi_im (alpha 0) * qi_im (alpha 0)) *
      (qi_re (beta 1) * qi_re (beta 1) + qi_im (beta 1) * qi_im (beta 1)) ==
      (qi_re (alpha 0) * qi_re (beta 1) - qi_im (alpha 0) * qi_im (beta 1)) *
      (qi_re (alpha 0) * qi_re (beta 1) - qi_im (alpha 0) * qi_im (beta 1)) +
      (qi_re (alpha 0) * qi_im (beta 1) + qi_im (alpha 0) * qi_re (beta 1)) *
      (qi_re (alpha 0) * qi_im (beta 1) + qi_im (alpha 0) * qi_re (beta 1))) by ring.
    rewrite Hkey. rewrite Hnorm01. rewrite Hnorm01i. ring. }
  ...
  - (* alpha(0) has norm 0 => alpha(0) = 0 *)
    ...
    assert (Har : qi_re (alpha 0) == 0).
    { assert (H1 : 0 <= qi_re (alpha 0) * qi_re (alpha 0)).
      { destruct (Qlt_le_dec (qi_re (alpha 0)) 0);
        [assert (Hh : (-qi_re (alpha 0)) * (-qi_re (alpha 0)) == qi_re (alpha 0) * qi_re (alpha 0)) by ring;
         rewrite <- Hh; apply Qmult_le_0_compat; lra |
         apply Qmult_le_0_compat; lra]. }
      assert (H2 : 0 <= qi_im (alpha 0) * qi_im (alpha 0)).
      { destruct (Qlt_le_dec (qi_im (alpha 0)) 0);
        [assert (Hh : (-qi_im (alpha 0)) * (-qi_im (alpha 0)) == qi_im (alpha 0) * qi_im (alpha 0)) by ring;
         rewrite <- Hh; apply Qmult_le_0_compat; lra |
         apply Qmult_le_0_compat; lra]. }
      assert (Hre2 : qi_re (alpha 0) * qi_re (alpha 0) == 0) by lra.
      destruct (Qeq_dec (qi_re (alpha 0)) 0) as [|Hne]; [assumption|].
      exfalso. assert (0 < qi_re (alpha 0) * qi_re (alpha 0)).
      { destruct (Qlt_le_dec (qi_re (alpha 0)) 0).
        - assert (Hh : qi_re (alpha 0) * qi_re (alpha 0) ==
            (-qi_re (alpha 0)) * (-qi_re (alpha 0))) by ring.
          rewrite Hh. apply Qlt_mult_pos_pos; lra.
        - assert (Hgt : 0 < qi_re (alpha 0)) by lra.
          apply Qlt_mult_pos_pos; lra. }
      lra. }
    rewrite Har in H00r. rewrite Qmult_0_l in H00r.
    assert (Hai : qi_im (alpha 0) == 0).
    { assert (H2 : qi_im (alpha 0) * qi_im (alpha 0) == 0).
      { unfold qi_norm2 in Ha0. rewrite Har in Ha0.
        assert (Hh : 0 * 0 == 0) by ring. rewrite Hh in Ha0. lra. }
      destruct (Qeq_dec (qi_im (alpha 0)) 0) as [|Hne]; [assumption|].
      exfalso. assert (0 < qi_im (alpha 0) * qi_im (alpha 0)).
      { destruct (Qlt_le_dec (qi_im (alpha 0)) 0).
        - assert (Hh : qi_im (alpha 0) * qi_im (alpha 0) ==
            (-qi_im (alpha 0)) * (-qi_im (alpha 0))) by ring.
          rewrite Hh. apply Qlt_mult_pos_pos; lra.
        - assert (Hgt : 0 < qi_im (alpha 0)) by lra.
          apply Qlt_mult_pos_pos; lra. }
      lra. }
  ...
  - (* beta(1) has norm 0 => beta(1) = 0 *)
    ...
    assert (Hbr : qi_re (beta 1) == 0).
    { assert (H1 : 0 <= qi_re (beta 1) * qi_re (beta 1)).
      { destruct (Qlt_le_dec (qi_re (beta 1)) 0);
        [assert (Hh : (-qi_re (beta 1)) * (-qi_re (beta 1)) == qi_re (beta 1) * qi_re (beta 1)) by ring;
         rewrite <- Hh; apply Qmult_le_0_compat; lra |
         apply Qmult_le_0_compat; lra]. }
      assert (H2 : 0 <= qi_im (beta 1) * qi_im (beta 1)).
      { destruct (Qlt_le_dec (qi_im (beta 1)) 0);
        [assert (Hh : (-qi_im (beta 1)) * (-qi_im (beta 1)) == qi_im (beta 1) * qi_im (beta 1)) by ring;
         rewrite <- Hh; apply Qmult_le_0_compat; lra |
         apply Qmult_le_0_compat; lra]. }
      assert (Hre2 : qi_re (beta 1) * qi_re (beta 1) == 0) by lra.
      destruct (Qeq_dec (qi_re (beta 1)) 0) as [|Hne]; [assumption|].
      exfalso. assert (0 < qi_re (beta 1) * qi_re (beta 1)).
      { destruct (Qlt_le_dec (qi_re (beta 1)) 0).
        - assert (Hh : qi_re (beta 1) * qi_re (beta 1) ==
            (-qi_re (beta 1)) * (-qi_re (beta 1))) by ring.
          rewrite Hh. apply Qlt_mult_pos_pos; lra.
        - assert (Hgt : 0 < qi_re (beta 1)) by lra.
          apply Qlt_mult_pos_pos; lra. }
      lra. }
    assert (Hbi : qi_im (beta 1) == 0).
    { assert (H2 : qi_im (beta 1) * qi_im (beta 1) == 0).
      { rewrite Hbr in Hb1. assert (Hh : 0 * 0 == 0) by ring.
        rewrite Hh in Hb1. lra. }
      destruct (Qeq_dec (qi_im (beta 1)) 0) as [|Hne]; [assumption|].
      exfalso. assert (0 < qi_im (beta 1) * qi_im (beta 1)).
      { destruct (Qlt_le_dec (qi_im (beta 1)) 0).
        - assert (Hh : qi_im (beta 1) * qi_im (beta 1) ==
            (-qi_im (beta 1)) * (-qi_im (beta 1))) by ring.
          rewrite Hh. apply Qlt_mult_pos_pos; lra.
        - assert (Hgt : 0 < qi_im (beta 1)) by lra.
          apply Qlt_mult_pos_pos; lra. }
      lra. }
    rewrite Hbr in H11r. rewrite Hbi in H11r.
    rewrite Hbi in H11i.
    assert (Hsimp : qi_re (alpha 1) * 0 - qi_im (alpha 1) * 0 == 0) by ring.
...
Definition entanglement_witness (psi : CompositeState) : Q :=
  qi_norm2 (psi 0 0) * qi_norm2 (psi 1 1) -
  qi_norm2 (psi 0 1) * qi_norm2 (psi 1 0).
